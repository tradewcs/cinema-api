...
MIGRATIONS_DIR="/app/src/alembic/versions"
...
# Use POSTGRES_USER from environment (should be 'cinema')
if ! psql -h "$POSTGRES_HOST" -U "$POSTGRES_USER" -d "$POSTGRES_DB" -c "\dt" | grep -q "alembic_version"; then
    ...
    # Run database populate script only if alembic_version table was just created
