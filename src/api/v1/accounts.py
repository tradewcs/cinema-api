...
@router.get(
    ...
    description="Activate a user's account using email and activation token from query params.",
...
async def login_user(
    ...
    try:
        refresh_token = RefreshToken.create(
            user_id=user.id,
            days_valid=settings.JWT_REFRESH_TTL,
...
async def resend_activation_email(
    ...
    stmt = select(ActivationToken).filter_by(user_id=user.id)
    result = await db.execute(stmt)
